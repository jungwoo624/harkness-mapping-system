...
export function SessionReport({ session }: SessionReportProps) {
  ...
  return (
    <section className="space-y-6 text-left" data-testid="session-report">
      ...
      {/* 전체 요약 카드 */}
      <div
        className="space-y-1.5 rounded-xl border border-gray-200 bg-white p-4"
      ...
      >
        ...
        {overall.isolatedStudents.length > 0 ? (
          <p
            className="rounded-lg bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-500"
          ...
          <p
            className="rounded-lg bg-teal-50 px-3 py-2 text-sm font-semibold text-teal-600"
